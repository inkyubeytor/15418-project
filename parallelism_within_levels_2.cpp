...
//                   omp_get_thread_num(), level_index, v);
            ...
            vector<pair<int, kdict_t>> local_table;
            local_table.push_back(make_pair(-1, parts_0));
            ...
            for (int child: children) {
                ...
                for (int k = 1; k < parts + 1; k++) {
                    ...
                    for (int k_prime = 1; k_prime < k + 1; k_prime++) {
                        zdict_t left = local_table.back().second[k_prime];
                        zdict_t right = dp_table[child].back().second[k -
                                                                      k_prime +
                                                                      1];
                        for (auto l_it = left.begin();
                             l_it != left.end(); ++l_it) {
                            cost_t a = l_it->first;
                            for (auto r_it = right.begin();
                                 r_it != right.end(); ++r_it) {
                                cost_t b = r_it->first;
                                if (!z_dict.count(a + b)) {
                                    set1_t s1;
...
#pragma omp critical
                local_table.push_back(make_pair(child, parts_i));
            }
#pragma omp critical
            {
                dp_table[v] = local_table;
                processed.insert(v);
            }
        }
        level_rank++;
